...
Instructions for AI agents working on SmartSaber.
...
| `smartsaber/utils.py` | String normalization, hashing, cache key helpers shared across modules. |
...
| `smartsaber/analysis_cache.py` | Disk-backed cache (`~/.smartsaber/analysis/`) -- avoids re-running librosa on re-imports. |
| `smartsaber/bs_cache.py` | Disk-backed BeatSaver search cache. |
| `smartsaber/yt_cache.py` | Disk-backed YouTube URL cache. |
| `smartsaber/playlist.py` | Generates Beat Saber `.bplist` playlist files. |
...
- Soft parity enforcement (~50% at chest/overhead, ~10% at waist to avoid forced upstrokes)
...
- Doubles (both hands same beat) share the same cut direction
- Near-simultaneous notes from different hands must not have opposing directions
- Cross-body diagonals (e.g. left hand cutting right) are suppressed (0.15x weight)
- Row 0 upstrokes are nearly eliminated (0.05x weight)
- Onset selection uses sliding windows for local representation + gap-filling to prevent silent holes
- Bass-heavy onsets get salience boost so bass lines aren't dropped
...
- `bs_cache.json` -- BeatSaver search results
- `analysis/` -- librosa audio analysis results (versioned; bumping `_CACHE_VERSION` in `analysis_cache.py` invalidates old entries)
- `.spotify_token_cache` -- Spotify OAuth token

The `--regen` flag skips BeatSaver search and YouTube download entirely, re-generating notes for existing maps using cached analysis.

### Testing

Unit tests are in `tests/`. Additional timing/analysis scripts in `scripts/` and the project root are used for manual validation against human-made BeatSaver maps. The `import/UnitTests.csv` file contains 16 diverse test tracks.
